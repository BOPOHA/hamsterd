...
func TestRunVersionAndFlagErrors(t *testing.T) {
	...
	if !strings.HasPrefix(stdout.String(), "hamsterd ") {
...
func TestRunCreatesFilesBeforeOccupiedListenerError(t *testing.T) {
	...
	for _, name := range []string{"config.json", "ca.crt", "ca.key"} {
		if _, err := os.Stat(filepath.Join(configHome, "hamsterd", name)); err != nil {
			t.Errorf("expected %s to be created: %v", name, err)
		}
	}
}

func TestRunReportsConfigurationAndCAErrors(t *testing.T) {
	setUserDirs(t)
	...
	t.Run("listen override", func(t *testing.T) {
		var stderr bytes.Buffer
		if code := run([]string{"-listen", "public.example:8080"}, &bytes.Buffer{}, &stderr); code != 1 {
			t.Fatalf("exit code = %d, want 1", code)
		}
...
func TestRunReportsCacheDirectoryError(t *testing.T) {
	dir := t.TempDir()
	cachePath := filepath.Join(dir, "not-a-directory")
  ...
  "cache": {
	...
	if !strings.Contains(stderr.String(), "cache error") {
		t.Fatalf("stderr = %q", stderr.String())
	}
}
...
func TestInformationHandler(t *testing.T) {
	...
	tests := []struct {
		...
		{path: "/", contentType: "text/plain; charset=utf-8", body: "hamsterd caching proxy\nCA certificate: /ca.crt\nHealth check: /healthz\n"},
	...
	if got := recorder.Header().Get("Content-Disposition"); got != `attachment; filename="hamsterd-ca.crt"` {
